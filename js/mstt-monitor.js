...
function format(value, format) {
    switch(format) {
        ...
        case "GB": return (value/1000/1000/1000).toFixed(2);
        case "MB": return (value/1000/1000).toFixed(2);
        case "KB": return (value/1000).toFixed(2);
        case "B": return value;
...
$(document).ready(function() {
    ...
    $('.button-convert-size').click(function() {
        ...
        if (format == 'disc' || format == 'memory' || format == 'network')  f = 'bytes';
        else f = 'bytes';

        if (convert_format === 'b' && f == 'bytes')  value = value * 8;
        else if (convert_format === 'B' && f == 'bits') value = value / 8;
        
        format_output.text(prefix)

        if (prefix.length == 1) { convert.text(value); return; }

        switch (prefix[0]) {
            case "K": convert.text((value / KIBI).toFixed(2)); break;
            case "M": convert.text((value / MEBI).toFixed(2)); break;
            case "G": convert.text((value / GIBI).toFixed(2)); break;
            case "T": convert.text((value / TEBI).toFixed(2)); break;
        }
    })
    ...
    $(".delete-client").click(function() {
        ...
        $.getJSON('api.php', { "api": "delete_client", "id": id }, function(data) {
            if (!data.error) window.location = "?page=clients";
            else alert("danger", "Error! ", data.message);
        })
    })
    ...
    $(".add-group").click(function() {
        ...
        $.getJSON('api.php', { "api": "group_exists", "group": group }, function(data) {
            if (!data.error) {
                if (!data.message) {
                    $panel = $("<div>").addClass("panel panel-primary checks-item").attr("data-check", group)
                ...
                } else alert("danger", "Error! ", "Group already exists");
            } else alert("danger", "Error! ", data.message);
        })
    })
    ...
    $(document).on("change", "select", function(ev) {
        ...
        if (f == "cmd") api = "edit_command";
        else if (f == "group") api = "edit_group";
        else return;

        $.getJSON('api.php', { "api": api, "id": id, "key": target, "value": opt.toLowerCase() }, function(data) {
            if (!data.error) alert("success", "Success! ", data.message);
            else alert("danger", "Error! ", data.message);
        })
    })
        ...
        if (f == "alert") {
            ...
            if (toggle) {
                ...
                $.getJSON('api.php', { "api": "add_command_group", "command": command, "group": group }, function(data) {
                    if (!data.error) {
                        alert("success", "Success! ", "added a command to the group.");
                        $($this.children()[index]).data("cmd", data.message[0])
                        $table = $this.closest(".panel").find("table")
                        $tr = $("<tr>")
                        $tr.attr("data-cmd", data.message[0])
                        $tr.append($("<td>").text(data.message[0]))
                        $tr.append($("<td>").text(data.message[1]))
                        $tr.append($("<td>").append($("<div>")
                            .addClass("input-group bootstrap-timepicker timepicker")
                            .append($("<input>")
                                .attr("type", "text")
                                .addClass("form-control input-small interval-picker")
                                .data("id", data.message[0])
                                .data("for", "group")
                                .data("target", "next_check")
                                .data("previous", "00:00:00")
                                .timepicker({
                                    maxHours: -1,
                                    showMeridian: false,
                                    showSeconds: true,
                            ...
                            .append($("<span>")
                                ...
                                .append($("<i>").addClass("glyphicon glyphicon-time")))))
                        $tr.append($("<td>")
                            .append($("<select>")
                                .data("for", "group")
                                .data("id", data.message[0])
    ...
    $(document).on("blur", "[contenteditable]", function(ev) {
        ...
        else if (f == "client") api = "edit_client";
        else if (f == "server") api = "edit_server";
        else if (f == "alert") api = "edit_alert";
        else return;
    ...
    $(".clickable-row").click(function() {
        ...
        if (!$(this).attr("contenteditable")) window.location = $(this).data("href");
    })
